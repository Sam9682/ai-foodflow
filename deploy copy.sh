...
if [ "$1" = "ps" ] || [ "$1" = "status" ]; then
    ...
    # Try localhost first (for local testing), then remote
    if curl -s -k https://localhost:$HTTPPORT/health > /dev/null 2>&1; then
        PROTOCOL="https"
        HOST="localhost"
    elif curl -s http://localhost:$HTTPPORT/health > /dev/null 2>&1; then
        PROTOCOL="http"
        HOST="localhost"
    elif curl -s -k https://swautomorph.com:$HTTPPORT/health > /dev/null 2>&1; then
        PROTOCOL="https"
        HOST="swautomorph.com"
    ...
    if [ -n "$PROTOCOL" ]; then
        echo "   ✅ Health Endpoint: Responding ($PROTOCOL://$HOST:$HTTPPORT)"
        
        # Test main endpoints
        if curl -s -k $PROTOCOL://$HOST:$HTTPPORT/main > /dev/null 2>&1; then
            echo "   ✅ Main Dashboard: Accessible"
        else
            echo "   ❌ Main Dashboard: Not accessible"
        fi
        
        if curl -s -k $PROTOCOL://$HOST:$HTTPPORT/docs > /dev/null 2>&1; then
        ...
        if curl -s -k $PROTOCOL://$HOST:$HTTPPORT/config/status > /dev/null 2>&1; then
        ...
        # Test menu API
        if curl -s -k $PROTOCOL://$HOST:$HTTPPORT/menu-items/1 > /dev/null 2>&1; then
    ...
    if [ -n "$PROTOCOL" ]; then
        ...
        echo "   🌐 Access: $PROTOCOL://$HOST:$HTTPPORT/main"
    else
